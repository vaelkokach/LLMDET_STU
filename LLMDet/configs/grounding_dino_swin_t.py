...
val_dataloader = dict(
    ...
    #     _delete_=True,
    #     type='ConcatDataset',
    #     # datasets=[lvis_val_dataset, student_behavior_val_dataset]))
    # 	datasets=[ student_behavior_val_dataset]))
...
#    dataset_prefixes=['lvis_minival', 'student_behavior_emotion'])
...
default_hooks = dict(
    checkpoint=dict(by_epoch=False, interval=1500, max_keep_ckpts=30),
    visualization=dict(type='GroundingVisualizationHook'),
    logger=dict(type='LoggerHook', interval=100))
log_processor = dict(by_epoch=False)
# Allow unused parameters when some branches (e.g., region LMM) are skipped.
...
env_cfg = dict(
    dist_cfg=dict(backend='nccl', timeout=3600), # 36000s = 10h
)
